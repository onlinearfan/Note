...
displayRecur(2); */
    ...
    //!html inside php,php inside html, javascript inside php,
    //form e sobar age submit click check korte hobe, click na kora porjonto sob tahole thik thakbe,
    // echo "<h2>html inside php</h2>" . "<script>alert('javascript inside php')</script>";
    //jetai kag kortesi oitai php inside html
    // www.php.com?fname=arfan&lname=khan&roll=12&address=72/1+s+k link of form submit

    //super global variable,
    // $_REQUEST,$GLOBALS,$_POST,$_GET,$_FILES,$_SESSION,$_COOKIE,$_SERVER,$_ENV
    //select e multiple e array name e pass korte hobe,
    //super global work between 2 file also, action jtai hobe setai show korbe, egulo sob array
    //alway use empty rather than isset,
    ...
    <!-- Handle text field -->
    <!-- //input = text,password,email,url,radio,select,checkbox, -->

    <!-- html code  -->
    <form action="">
        <h5>Simple text field</h5>
        <input type="text" name="textField" value=""><br>
        <input type="password" name="passwordField" value=""> <br>
        <h5>Radio Btn value take by value inside</h5>
        male:<input type="radio" name="gender" id="" value="m">
        female:<input type="radio" name="gender" id="" value="f">
        Other:<input type="radio" name="gender" id="" value="O">
        <h5>checkbox : value take by inside value</h5>
        PHP:<input type="checkbox" value="php" name="languageCheckbox">
        C++: <input type="checkbox" value="c++" name="languageCheckbox">
        java: <input type="checkbox" value="java" name="languageCheckbox">
        ...
        <h5>Submit Button will check by click</h5>
  ...
  /*   if (!empty($_REQUEST['submit'])) { //eita check kore var e sob gulo value sotre korte parbo,
        if (!empty($_REQUEST['textField']) && !empty($_REQUEST['passwordField']) && !empty($_REQUEST['gender']) && !empty($_REQUEST['languageCheckbox']) && !empty($_REQUEST['selectField'])) {
            // echo $_REQUEST['selectField'][1];
            // echo $_REQUEST['gender'];
            // echo $_REQUEST['languageCheckbox'];

            print_r($_REQUEST['selectField']);
        } else {
            echo "Fill all form properly";
        }
    } */
    ...
    print__r($_REQUEST);
    //$_REQUEST er bitor from er sob data cole ase ek bare tai amra eita foreach korte pari,
